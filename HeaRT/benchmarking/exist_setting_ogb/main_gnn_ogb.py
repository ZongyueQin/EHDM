...
def train(model, score_func, split_edge, train_pos, data, emb, optimizer, batch_size, pos_train_weight, data_name, remove_edge_aggre):
    ...
    for perm in tqdm(DataLoader(range(train_pos.size(0)), batch_size,
                           shuffle=True)):
        optimizer.zero_grad()
...
def main():
    ...
    if args.data_name == 'ogbl-citation2' or 'igb' in args.data_name: 
    ...
    for run in range(args.runs):
                ...
                if best_valid_current > best_valid:
                    ...
                    if args.edge_path_file != 'None' and run == 0:
                        if args.production == False:
                            results_distill, pos_pred, neg_pred = get_score(model, score_func, pos_edge, neg_edge, data, data.x, evaluator_hit, evaluator_mrr, args.batch_size)
                        else:
        ...
        if args.edge_path_file != 'None':
            ...
            score_func.load_state_dict(best_state['predictor'])
            if args.production == False:
                results_distill, pos_pred, neg_pred = get_score(model, score_func, pos_edge, neg_edge, data, data.x, evaluator_hit, evaluator_mrr, args.batch_size)
            else:
                raise NotImplementedError
    ...
    print(f"train time spent seconds: {np.mean(time_spent):.2f}± {np.std(time_spent):.2f}")

    if args.production == False:
        if args.edge_path_file != 'None':
            torch.save((pos_pred, neg_pred), f'{args.data_name}_{args.gnn_model}_pred.pth')
        torch.save(best_states, f'/home/qinzongyue/linkless-link-prediction/src/saved_students/{args.data_name}_{args.gnn_model}_{args.hidden_channels}.pth')
    else:
        if args.edge_path_file != 'None':
