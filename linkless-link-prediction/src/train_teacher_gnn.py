...
@torch.no_grad()
def see_train_test_diff(model, predictor, data, split_edge, evaluator, batch_size, encoder_name, dataset, args, detail=False):
    model.eval()
    predictor.eval()
    ...
    if dataset != "collab":
        neg_edge = negative_sampling(edge_index, num_nodes=data.x.size(0),
                             num_neg_samples=pos_train_edge.size(0), method='dense')
    elif dataset == "collab":
        neg_edge = torch.randint(0, data.x.size()[0], pos_train_edge.size(), dtype=torch.long,
                         device=h.device)
    pos_train_preds = []
    for perm in DataLoader(range(pos_train_edge.size(0)), batch_size):
        edge = pos_train_edge[perm].t()
...
@torch.no_grad()
def test_transductive(model, predictor, data, split_edge, evaluator, batch_size, encoder_name, dataset, args, detail=False):
    model.eval()
    predictor.eval()
    ...
    if detail == False:
        ...
        return results, h, pos_pred, pos_edge, neg_pred, neg_edge
        ...
        for epoch in range(1, 1 + args.epochs):
            if args.transductive == "transductive":
                ...
                results, h, pos_pred, pos_edge, neg_pred, neg_edge = test_transductive(model, predictor, data, split_edge,
                            evaluator, args.batch_size, args.encoder, args.datasets, args, detail=True)
            else:
